...
BASE_URL = "https://balardi.com"


graphql_url = "https://balardi.myshopify.com/api/unstable/graphql.json"
headers = {
    "Content-Type": "application/json",
    "Accept": "*/*",
    "Origin": "https://balardi.com",
    "Referer": "https://balardi.com",
    "User-Agent": "Mozilla/5.0",
    "x-shopify-storefront-access-token": "7538dcaafbb5323afa63f6f4b515ce24"
}
    ...
    for product in products:
        ...
        if gender_tag:
            ...
            if gender_lower == "men":
                ...
                normalized_tags = [tag.lower() for tag in product_tags]

                # Check for any tag that contains 'cloth' as substring
                if any("cloth" in tag for tag in normalized_tags):
                    gender_tags = {"all clothing men", "mens", "men clothing", "men"}
                else:
                    gender_tags = {"men", "mens", "men's"}
        ...
        type_val = re.sub(r"\bmen'?s?\b", "", type_val, flags=re.IGNORECASE).strip()
        type_val = re.sub(r"\s{2,}", " ", type_val)
...
def complete_workflow_balardi():

    collections = [
        {"url": "https://balardi.com/collections/sale-for-men", "gender": "men"},
    ]
