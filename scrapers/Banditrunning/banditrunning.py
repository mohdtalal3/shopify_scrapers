...
def complete_workflow(collections, limit_products=None, output_json=False):
    ...
    if limit_products:
        unique_handles = unique_handles[:limit_products]
        print(f"🎯 Limited to {len(unique_handles)} products for testing")
    else:
        print(f"🎯 Total Unique Product Handles: {len(unique_handles)}")
    ...
    if output_json:
        ...
        print(f"✅ {len(unique_products)} products saved to {output_file}")
        
        # Also print first product as sample
        if unique_products:
            print("\n📋 Sample Product Data:")
            print(json.dumps(unique_products[0], indent=2, ensure_ascii=False))
...
if __name__ == "__main__":
    ...
    complete_workflow(collections, limit_products=None, output_json=False)