...
SITEMAP_URL = "https://www.katespade.com/sitemap_0-product.xml"
load_dotenv()
...
def fetch_product_details(ids_list, batch_size=20, batches_per_session=20):
...
def clean_katespade_data(data):
    ...
    for product in data.get("productsData", []):
        ...
        # 🚨 Remove product if no variants found
...
def complete_workflow_kate():
    ...
    with open("kate_cleaned_sb.json", "w", encoding="utf-8") as f:
        json.dump(cleaned, f, indent=2, ensure_ascii=False)

    print(f"✅ Saved {len(cleaned)} cleaned products to kate_cleaned_sb.json")
