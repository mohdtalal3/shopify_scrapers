...
BASE_URL = "https://skimsbody.com"


graphql_url = "https://skimsbody.myshopify.com/api/unstable/graphql.json"
headers = {
    "Content-Type": "application/json",
    "Accept": "*/*",
    "Origin": "https://skims.com",
...
def clean_and_save_product_data_only_available_with_all_images_from_data(
    ...
    for product in products:
        ...
        normalized_tags = [t.lower() for t in product_tags]

        gender_tags = set()

        def has_men_tag(tags):
            for t in tags:
                # Match whole words like men/mens/men's/man but avoid matching 'women'
                if re.search(r"\bmen\b", t) or re.search(r"\bmens\b", t) or re.search(r"\bmen['’]s\b", t):
                    return True
                if re.search(r"\bman\b", t) or re.search(r"\bman's\b", t):
                    return True
            return False

        def has_women_tag(tags):
            for t in tags:
                # Matches woman/women/womens/women's
                if re.search(r"\bwom[ae]n('s|s)?\b", t):
                    return True
            return False
        gender_tag=None
        if has_men_tag(normalized_tags):
            gender_tag="men"
            gender_tags.update(["all clothing men", "mens", "men clothing", "men"])

        if has_women_tag(normalized_tags):
            gender_tag="women"
            gender_tags.update(["all clothing women", "womens", "women clothing", "women"])
...
def complete_workflow_skims():

    collections = [
        {"url": "https://skims.com/en-pk/collections/skims-sale", "gender": "women"},
    ]
